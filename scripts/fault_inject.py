...
def ssh_cmd(target_host, cmd):
    ...
    client.connect(hostname=target_host, username='root', password='openeuler@123')
    try:
        c = ""
        for c in cmd:
...
def check_mysql_running(mysql_ip):
    ...
    client.connect(hostname=lvs_ip, username='root', password='openeuler@123')
...
def check_tpcc_status(tpcc_ip):
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(hostname=tpcc_ip, username='root', password='openeuler@123')
    try:
